...
def process_configs(lines: List[str], precise=False) -> List[str]:
    ...
    def worker(line):
        # حذف خطوط خالی یا با پین خراب
        if not line or "pin=0" in line.lower() or "pin=red" in line.lower():
            return
        passed = precise_test(line) if precise else True
        if passed:
            with lock:
...
if __name__ == "__main__":
    ...
    print(f"[*] {len(all_lines)} lines fetched and duplicates removed.")

    # مرحله اول: ذخیره اولیه normal.txt
    ...
    # مرحله دوم: تست دقیق نهایی و ذخیره در final.txt و سایر خروجی‌ها
    print("[*] Running precise tests on normal configs for final output...")
