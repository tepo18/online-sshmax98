...
TIMEOUT = 3  # ثانیه برای تست اتصال
...
def is_port_open(host: str, port: int, timeout=TIMEOUT) -> bool:
    try:
...
def test_config(cfg: Dict) -> bool:
    # بررسی اتصال واقعی برای هر outbound
...
def update_subs():
    ...
    # ذخیره نهایی تنها در final.json
    with open(FINAL_JSON, "w", encoding="utf-8") as f:
        json.dump(final_list, f, indent=4, ensure_ascii=False)

    print(f"[✅] Updated {FINAL_JSON} ({len(final_list)} configs passed connectivity test)")

if __name__ == "__main__":
    print("[*] Starting advanced JSON subscription updater with connectivity test...")
    update_subs()
